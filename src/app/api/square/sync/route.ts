...
export async function POST(request: NextRequest) {
  try {
    ...
    // Backfill invoiceActivity for currently UNPAID invoices so the dashboard buckets
    // (Viewed / Sent-Not-Viewed / Not-Sent) reflect Square's source of truth.
    // We key by current month — invoiceActivity tracking is per-month.
    const now = new Date();
    ...
    for (const invoice of unpaidInvoices) {
      ...
      const existingActivity = (parent.data.invoiceActivity as Record<string, unknown> | undefined)?.[currentMonthKey] as
        | { squareInvoiceId?: string; sentAt?: string | null; viewedAt?: string | null; viewCount?: number; lastReminderAt?: string | null }
        | undefined;

      // Only backfill if we have nothing OR the invoiceId changed (don't clobber sentAt/viewedAt for the same invoice)
      if (existingActivity?.squareInvoiceId === invoice.id) continue;

      const amt = invoice.paymentRequests?.[0]?.computedAmountMoney?.amount;
      const amount = amt ? Number(amt) / 100 : 0;

      const newActivity = {
        squareInvoiceId: invoice.id || '',
        publicUrl: invoice.publicUrl || '',
        amount,
        // We don't know if it was sent — leave null so it shows as "Not Sent" until the user texts via dashboard
        sentAt: existingActivity?.sentAt ?? null,
        viewedAt: null,
        viewCount: 0,
        lastReminderAt: null,
      };
      ...
      await updateDoc(parentRef, {
        [`invoiceActivity.${currentMonthKey}`]: newActivity,
      });
      activityBackfilled++;
    }
